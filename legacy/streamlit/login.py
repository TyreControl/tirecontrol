...
def check_login(username, password):
    ...
    if user:
        ...
        if hash_password(password) == password_hash:
            role_normalizado = str(user.get('role') or '').strip().lower()
            if not role_normalizado:
                role_normalizado = 'operador'

            # Salva informacoes na sessao
            st.session_state['logged_in'] = True
            st.session_state['user_id'] = user['id']
            st.session_state['user_name'] = user['nome']
            st.session_state['user_role'] = role_normalizado
            # Salva o cliente_id na sessao para uso global
