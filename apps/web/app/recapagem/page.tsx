...
export default function RecapagemPage() {
    ...
    const loadOrders = async (clienteId: string) => {
        // Fetch orders that are not fully completed/archived if needed, 
        // but typically we show all active ones. Legacy showed 'enviado' and 'em_processo'.
        // Let's show all that are NOT 'concluido' or maybe just list recent ones.
        // Legacy: status = 'enviado' or 'em_processo'
        const { data } = await supabase
    ...
    const handleCreateOrder = async () => {
        ...
        try {
            // Get Marca Fogo codes for selected IDs (RPC expects codes or we can refactor RPC to take IDs? 
            // RPC definition: p_codigos text[]. It matches by marca_fogo.
            // Let's get the Codes from the selected IDs.
    ...
    const handleReceiveOrder = async (orderId: string, items: Pneu[]) => {
        ...
        try {
            ...
            loadOrders(userProfile.clienteId)
            loadStockTires(userProfile.clienteId) // They should be back in stock
            setExpandedOrder(null)
